...
[View our pitch deck →](https://drive.google.com/file/d/13jucVg4iM-MGIh-oAOYQhNu9rEgHqfZe/view?usp=sharing)

---

## Demo (Prototype)

[shieldpay-prototype.vercel.app](https://shieldpay-prototype.vercel.app)

---

## Demo Video
https://drive.google.com/file/d/1CBfNqXX0XXTFVF6P14GSVq09whpVUln6/view?usp=sharing
...
## What makes ShieldPay stand out

Most parametric insurance prototypes demonstrate the concept. ShieldPay demonstrates the system — end to end, both sides of the market, fully automated, fully deployed.

**Truly automatic, not simulated automatic**
A live rainfall gauge watches OpenWeatherMap data for the worker's zone in real time. When rainfall crosses the threshold, the system detects it, verifies two independent signals, and fires the payout automatically. No button pressed. No claim filed. No human in the loop. The pipeline runs itself. That is what parametric actually means — and it is demonstrable live in this prototype.

**Four distinct worker roles, not one generic gig worker**
A cold chain inventory associate and a delivery rider face completely different risks. ShieldPay prices them differently, covers them differently, and pays them differently. Delivery partners get full weather and AQI coefficients. Pickers and packers working indoors get 40% of the weather coefficient — their loss is real but partial. Inventory associates managing cold chain get a 90% power outage coefficient — because a power cut at a cold store is a fundamentally different event. Role-adjusted coefficients built from the ground up for the operational reality of dark store work.

**Two complete personas — worker and insurer**
ShieldPay is not a worker app. It is a two-sided insurance system. The worker side handles onboarding, AI-calculated premiums, live zone monitoring, automated payouts, and ShieldCredit score building. The insurer side shows live loss ratios, zone-by-zone financial breakdown, fraud flag queue, next-week risk forecast, and a live rainstorm demo trigger. Both personas are fully functional and deployed right now.

**A financial identity built as a side effect of working**
ShieldCredit is not a loyalty programme. It is a credit score built from verified insurance behaviour — premium payments made on time, claims filed honestly, income earned and tracked. 50 million gig workers in India are credit invisible. ShieldPay fixes that as a side effect of just showing up to work. Three clean quarters unlocks a pre-approved emergency credit line. The worker never applied for credit. They just did their job.

**Fraud-resistant by architecture, not just detection**
Because payouts are triggered by an external independent index — not a worker's self-report — the entire category of claims fraud is structurally eliminated. What remains is handled by a three-layer system: GPS spoofing detection, claim velocity analysis, and live weather cross-validation. All three feed into a confidence score from 0–100 with tiered response. We never block honest workers. We tier.

---

## Phase 3 — What's new

Phase 3 closes every gap identified in Phase 2 feedback. The judges said ShieldPay fell short on automation and real-time data integration. Here is the answer.

**Live parametric engine**
A rainfall threshold meter watches OpenWeatherMap data for the worker's zone in real time. When rainfall crosses 50mm/hr, the system detects it, verifies the second signal, and fires the payout automatically. No button pressed. No claim filed. No human in the loop. This is now demonstrable end to end in the prototype.

**Advanced fraud detection**
Three-layer system: GPS spoofing detection (worker location vs registered 2km zone), claim velocity analysis (8+ payouts in 30 days flags syndicate patterns), and live weather cross-validation (OpenWeatherMap confirms a disruption actually occurred in the claimed zone at the claimed time). All three feed into a confidence score from 0–100. The score determines payout speed — not whether the payout happens.

**Instant payout system**
Razorpay test mode integrated with real sandbox API calls. UPI reference generated per payout. Full receipt: transaction ID, UPI reference, order drop percentage confirmed, timestamp. Not mocked — real API calls to Razorpay's sandbox environment.

**Intelligent insurer dashboard**
A complete second persona built for the insurer/operator. Live loss ratios by zone and tier. Zone-by-zone premium vs disbursement breakdown. Fraud flag queue. Next-week risk forecast using Bayesian probability updates from 30-day trigger history. Everything an insurer needs to actually underwrite this product.

| Phase 2 | Phase 3 |
|---|---|
| "Fire trigger" button | Live rainfall gauge — auto-fires on threshold breach |
| Razorpay mocked | Razorpay test mode — real sandbox API calls |
| Basic fraud ratio check | GPS spoofing + velocity + weather cross-validation → confidence score 0–100 |
| Worker app only | Worker app + full insurer dashboard |
| No external API integration | OpenWeatherMap live, Razorpay sandbox |
| Manual simulation only | Automated parametric pipeline end to end |
...
Parametric insurance removes all of that. We don't measure the worker's loss directly. We measure an independent external index — rainfall in mm/hr from OpenWeatherMap (proxying IMD data for the prototype; direct IMD API integration in production), AQI from CPCB, a disaster advisory from NDMA — and when that index crosses a pre-agreed threshold, the payout fires automatically.

The trigger is a publicly verifiable, tamper-proof number from an independent data source. The worker doesn't prove anything. We don't investigate anything. The math runs, the money moves.
...
|---|---|---|---|---|
| Heavy rainfall | Partial: >15mm/hr | 50% | 0.55 | OpenWeatherMap (IMD in production) |
| Heavy rainfall | Full: >50mm/hr | 100% | 0.80 | OpenWeatherMap (IMD in production) |
...
The weekly premium is not looked up from a table. It is computed fresh for every active worker using ML models chained together:

```
STEP 1 — Model 1 (GradientBoostingRegressor income baseline)
...
STEP 2 — Disruption probability table
Input:  historical trigger frequency per city, updated with 30-day Bayesian adjustment
Output: P(trigger fires this week) per trigger type

          ↓

STEP 3 — Premium formula (deterministic, not ML)
...
### Model 1 — Income baseline (GradientBoostingRegressor)
...
This prevents a new worker from being immediately disadvantaged by a low cohort median if their actual earnings are higher, while protecting the pool from inflated baselines if a worker overstated their income at enrollment.

### Model 2 — Disruption probability (Bayesian update)

**What it does:** Uses historical trigger frequency per city, updated with 30-day rolling trigger history, to produce next-week probability estimates per trigger type per zone.

**Why it matters:** Two uses. First: feeds into next week's premium calculation — a Bengaluru worker's premium is higher the week before a predicted monsoon surge. Second: powers the insurer dashboard's predictive analytics tab — city-by-city risk forecast with reserve recommendations.

### Model 3 — Fraud detection (Isolation Forest + confidence scoring)

**What it does:** Scores each claim event across three independent signal layers. GPS zone validation, claim velocity analysis, and live weather cross-validation from OpenWeatherMap. All three combine into a confidence score from 0–100 with tiered payout response.

**Inputs:** Worker GPS coordinates, claim frequency history, OpenWeatherMap live data for zone.
**Output:** Confidence score 0–100 → tiered payout decision.

**Important:** The Isolation Forest is the third layer of fraud detection, not the first. The structural layers come first.
...
Because payouts are triggered by an external independent index — not a worker's self-report — the entire category of claims fraud is structurally eliminated. Nobody can fake an OpenWeatherMap rainfall reading or a CPCB AQI station output.
...
High fraud score → payout held in a soft hold queue for manual review. Not denied — held.

### Pillar 2 — Location and activity validation

**GPS zone checkpoint:** On a trigger day, when a payout is being processed, one GPS check is performed — was the worker's app active and GPS-present within their registered 2km zone during the trigger window?
...
**Activity validation:** Order completion rate during the trigger window is cross-referenced with platform data. If a worker completed all their active orders on time during the trigger window, no income loss occurred and no payout fires — regardless of whether the external trigger index was breached. This is the two-signal rule in action.
...
Every payout is assigned a confidence score from 0–100 based on the full signal vector.
...
**Coordinate entropy.** Genuine workers spread naturally across a zone. Spoofers cluster. If thirty workers have GPS coordinates within fifty metres of each other, that is a spoofing app's default drop pin, not a dark store zone.

**Timing distribution.** When a real disruption hits, workers trickle into the affected area over fifteen to thirty minutes as conditions develop. A syndicate coordinates on Telegram and acts simultaneously. A spike of forty location pings within a ninety-second window at trigger onset is a syndicate signature.

**Device fingerprint graph.** The same device model, Android version, and mock location app signature appearing across multiple worker accounts is a network edge. We map these edges. Connected components that activate together during triggers are syndicate candidates.

**Payout velocity.** A worker who successfully claims on every single trigger event in their city, across multiple zone types and shifts, is statistically implausible. Genuine workers miss triggers. A perfect claim record over time is a red flag, not a loyalty signal.
...
A genuine rider in a heavy rainstorm will have degraded GPS. They may be sheltering under a flyover, which causes a cell tower mismatch. These signals overlap with fraud signals. A binary block would punish them.
...
- Razorpay test mode integrated — real sandbox API calls, not mocked
- UPI direct transfer to worker's registered UPI ID
- Target: payout within 2 hours of trigger confirmation
- Full receipt generated: transaction ID, UPI reference, order drop % confirmed, timestamp
- Payout history endpoint — worker sees all historical payouts with amounts and dates
- Vespa animation + thermal receipt UX on payout confirmation — the money arrives before the worker gets home
...
### Worker view
- Coverage status — active, tier, zone
- Live zone weather — OpenWeatherMap live feed for registered zone with trigger status
- Earnings protected — total income protected across all payouts received, pulled from DB in real time
- Weekly coverage ratio — tier coverage percentage
- Payout history — itemised, real data from DB
- ShieldCredit score panel — streak, discount progress, credit eligibility

### Insurer / Admin view
- Overview — total workers, active policies, loss ratio, fraud hold rate
- Financial summary — total premium collected vs total disbursed
- Zone breakdown — per-zone workers, premium, disbursed, loss ratio
- Predictive analytics — next-week trigger probability per city, Bayesian-adjusted from 30-day history, reserve recommendation per city
- Fraud flag queue — all held payouts with fraud scores, worker IDs, amounts
- Rainstorm demo trigger — select any zone, fire a simulated full rainstorm, watch the parametric pipeline run end to end in real time
...
|---|---|---|
| 1 | Phone number (10 digits, validated) | Identity. |
| 2 | Platform | Determines cohort and coefficient table. |
| 3 | Platform worker ID — auto-formatted (ZPT-, BLK-, INS-, BBN-) | Format-validated at input. Prevents random entries. |
| 4 | Role | Determines which coefficient multipliers apply. |
| 5 | Dark store zone | Sets the 2km geofence for trigger monitoring. |
| 6 | Shift window | Determines which hours trigger monitoring is active. |
...
| 8 | Tier selection | Three tiers shown with their specific calculated weekly premium. |
| 9 | Coverage activated | Earn first, pay Monday. |
...
The trigger monitoring pipeline watches OpenWeatherMap data continuously. When a threshold is crossed, the two-signal check runs immediately. If both signals confirm, the payout pipeline initiates automatically.

**From trigger confirmation to UPI transfer: target under 2 hours.**

```
OpenWeatherMap API (live rainfall per zone)
         ↓
Threshold engine — partial (>15mm) or full (>50mm) trigger classification
         ↓
Two-signal check — dark store order drop >40% in zone?
  ...
  No payout           GPS zone checkpoint
                           ↓
                    Activity validation
                    ↓ Normal orders        ↓ Income loss confirmed
                  No payout           Fraud confidence score (0–100)
                              ↓ <25              ↓ ≥75
                         Human review       Auto-approved
                                                 ↓
                                       Razorpay Payout API
                                                 ↓
                                    UPI transfer + receipt + push notification
```
...
Workers set up a Razorpay UPI AutoPay mandate once at onboarding. Every Monday morning, after the worker's weekly platform payout has settled, the premium is deducted automatically.
...
Razorpay Payout API (test mode) transfers to the worker's registered UPI ID within 2 hours of trigger confirmation. Full receipt generated with transaction ID, UPI reference, order drop confirmation, and timestamp.

---

## Dark store granularity

ShieldPay prices and triggers at 2km resolution — not city level, not district level.
...
**Store-level micro risk pool:** Workers at the same dark store share a visible micro pool. Their dashboard shows the store's collective loss ratio, number of enrolled workers, and pool health.

**Cold chain power coefficient:** Inventory associates at stores with significant cold chain infrastructure receive a 0.90 power outage coefficient instead of the standard 0.55.

---

## ShieldCredit Score — building financial identity
...
50 million gig workers in India are credit invisible. No salary slips. No formal employment. No loan history. Banks cannot reach them.
...
Discounts are applied to the following quarter's premium. Legitimate claims do not reset the streak or affect the ShieldCredit Score. A worker who claims what they are owed is not penalised. Only fraudulent flags affect the score.

The NBFC credit line is referred, not issued by ShieldPay. We share the worker's payment record (with explicit consent) with an NBFC partner who issues the line and pays ShieldPay a referral fee per activation.
...
If a worker accepts an order outside their registered shift window and a trigger fires before the order is completed, they receive coverage for that order's estimated income value. Verifiable via order acceptance timestamp.
...
| Dark store B2B hedge | Zepto / Blinkit pay a monthly premium to cover inventory spoilage on trigger days. | High ACV, low marginal cost. |
...
> *"ShieldPay is the first chronologically parametric insurance platform in India — priced at dark store granularity, verified by two independent signals, and purpose-built for the 10-minute delivery economy."*
...
- Push notifications are non-negotiable. Trigger alerts, payout confirmations, and pre-event surge upgrade prompts must reach workers in real time.
- UPI AutoPay mandate setup requires a handoff to the worker's UPI app — a native mobile flow.
- The single passive GPS checkpoint for zone validation requires device location permissions.
- The entire financial life of a gig worker — UPI, platform earnings, bank notifications — is already on their phone.

**The admin dashboard is web-only.** Operators reviewing fraud queues, monitoring loss ratios, and watching live trigger events do this at a desk. Two separate surfaces, one backend.

**Production:** React Native with Expo — iOS and Android from a single codebase.
**Prototype:** React + Vite, mobile-responsive, runs in phone browser.

---

## Tech stack

### Prototype (Phase 3 — current)

| Layer | Technology | Status |
|---|---|---|
| Frontend | React + Vite (web, mobile-responsive) | Live on Vercel |
| Backend | FastAPI + SQLAlchemy + SQLite | Live on Railway |
| ML — income baseline | GradientBoostingRegressor (scikit-learn) | Live |
| ML — fraud detection | Isolation Forest + confidence scoring | Live |
| ML — next-week forecast | Bayesian probability update from 30-day history | Live |
| Payments | Razorpay test mode — real sandbox API calls | Live |
| Weather | OpenWeatherMap live API | Live |
| Trigger monitoring | Live rainfall gauge — auto-trigger on threshold breach | Live |
| Fraud detection | GPS spoofing + velocity + weather cross-validation | Live |
| Admin dashboard | Full insurer view — loss ratios, forecast, fraud flags, demo trigger | Live |
| CI/CD | GitHub → Vercel + Railway auto-deploy | Live |
...
| Cache | Redis (Upstash) |
| ML models | XGBoost + Prophet + sklearn + Isolation Forest with real training data |
| Trigger monitoring | APScheduler polling IMD/CPCB/NDMA every 30 min |
| Payments | Razorpay Subscriptions + Payout API with webhooks |
| External APIs | IMD (free), CPCB (free), NDMA (free), OpenWeatherMap |
...
AQI crosses 420 at 9pm during Arjun's night shift. CPCB feed triggers. Delhi north zone order volume drops 71%. Two-signal confirmed. GPS shows Arjun in his registered zone. Fraud confidence score: 88/100 — auto-approved. Payout: ₹950 × 0.45 × 1.00 × 0.70 = ₹299.25. Transferred to his UPI by 11pm. He gets a push notification and goes to sleep knowing the night wasn't a total loss.

### Scenario 2 — Meena, Blinkit picker, Chennai, July
120mm of rain in 3 hours. Full rain trigger. Chennai-Adyar store order volume drops 68%. Two-signal confirmed. Meena is indoors — role multiplier 0.40 applies. Fraud confidence score: 91/100 — auto-approved. Her payout: ₹700 × 0.55 × 0.40 × 0.70 = ₹107.80. Smaller than a rider's payout, but accurate — she was sheltered. She still receives what she's owed.

### Scenario 3 — Sundar, BigBasket Now inventory associate, Bengaluru, any season
DISCOM power outage in his zone for 4.5 hours. Cold chain at his store starts warming. Two-signal confirmed (order volume collapsed — no picking, no packing, no dispatch). Power outage coefficient for inventory associates: 0.90. Fraud confidence score: 85/100 — auto-approved. His payout: ₹750 × 0.90 × 0.90 × 1.00 (Premium tier) = ₹607.50. The cold chain clause recognised his specific vulnerability. No other insurance product would have.

### Scenario 4 — Priya, 3 clean quarters, Zepto packer, Mumbai
Priya has had zero trigger payouts for 9 consecutive weeks — three full quarters. She receives a notification: *"You've gone 3 clean quarters. Your Q4 premium is 25% lower. And your ShieldCredit Score just unlocked a ₹7,500 emergency credit line."* She didn't plan for this. ShieldPay built her financial identity as a side effect of her doing her job.
...
- Somanathan et al. (2021). *The Impact of Temperature on Productivity and Labor Supply: Evidence from Indian Manufacturing.* Journal of Political Economy, 129(6).
  https://www.journals.uchicago.edu/doi/10.1086/713733
  — Documents measurable output loss and absenteeism on high-heat days in Indian labour-intensive industries. Directly relevant to our heat index trigger.

- Colmer, J. (2021). *Temperature, Labour Reallocation, and Industrial Production: Evidence from India.* American Economic Journal: Applied Economics, 13(4).
  https://www.ideasforindia.in/topics/productivity-innovation/temperature-labour-reallocation-and-industrial-production
  — Estimates a 1°C within-district temperature increase is associated with a 12% reduction in agricultural production and a 7-percentage point drop in employment.

**On AQI and gig workers specifically**

- Siddiqui, Sakunia & Mahmud (2024). *Riders in the smog: Pollution is poisoning gig workers.* Rest of World.
  https://restofworld.org/2024/riders-in-the-smog-gig-workers-pollution/
  — Rest of World equipped delivery riders in New Delhi, Lahore, and Dhaka with personal air quality monitors. Readings across shifts were significantly above WHO safety limits. 25 workers interviewed, all reported symptoms consistent with chronic pollutant exposure. The most directly relevant external study to our AQI trigger design.

- Newsclick (2023). *Gig Workers Amid Rising Air Pollution.*
  https://www.newsclick.in/gig-workers-amid-rising-air-pollution
  — Covers the disproportionate impact of poor AQI on platform workers in Delhi NCR and the absence of any employer protection for outdoor gig workers.
...
- LSE Business Review (2025). *How air pollution is holding back India's economy.*
  ...
  — Describes air pollution as a highly regressive tax on labour, reducing earnings precisely among those who are least able to bear the loss. Estimates ₹36.8 billion in economic losses from premature mortality and morbidity in India in 2019 alone.

**On the q-commerce sector and gig worker vulnerability**

- Business Standard Editorial (2026). *Gig workers' strike exposes the hard limits of India's quick commerce model.*
  ...
  — Cites a NITI Aayog 2024 survey finding that 90% of gig workers had no savings. Platform disconnection policies mean a single bad week — caused by weather, disruption, or illness — can erase a month of progress.
...
- IBEF (2026). *The Evolution of Quick Commerce in India: A Sectoral Analysis.*
  ...
  — Q-commerce GMV in India reached approximately ₹65,000 crore in FY25. The sector employs 250,000–300,000 delivery partners. Kearney estimates 62–64 jobs created per ₹100 crore GMV — the highest employment intensity of any retail format.
...
- Drishti IAS (2025). *Rise of Quick Commerce in India.*
  ...
  — Documents labor protection gaps, absence of insurance, and earnings volatility for dark store workers. Notes that riders earn ₹40–50 per delivery with no fixed income floor.
```